...
export async function POST(request: Request) {
  try {
    ...
    // Use provided recipients or get all active subscribers
    ...
    if (recipients && Array.isArray(recipients) && recipients.length > 0) {
      ...
      // Fallback to all active subscribers
    ...
    for (let i = 0; i < targetEmails.length; i += batchSize) {
      ...
      try {
        await resend.emails.send({
          from: 'Rise for Impact <newsletter@riseforimpact.org>',
          to: batchEmails,
          subject: subject,
          html: `
            <!DOCTYPE html>
        ...
        console.error(`Error sending to batch ${i / batchSize + 1}:`, error)
